...
func _check_pushed_rock_kills_enemy() -> void:
	...
	world.add_child(pusher)
	await _settle_frames(3)
	target.state_machine.change_state(&"RightDash", target.build_dash_record(Vector2.RIGHT))
	target.set_physics_process(false)
	pusher.state_machine.change_state(&"Dash", pusher.build_dash_record(Vector2.LEFT))
